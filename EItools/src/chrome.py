...
class ChromeCrawler:
        ...
        try:
            ...
            if 'www.google.com/recaptcha' in source_code:
                ...
                for clickCount in range(1,6):
                    ...
                    self.driver.excute_script("arguments[0].click();",element)
    ...
    def parse_page(self,gpage):
        ...
        if not snippets and "did not match any documents" not in gpage:
            logger.error("%s parse error for page %s",self.name,gpage)
            time.sleep(20)
        ...
        try:
            ...
            if type(e) is NoSuchElementException:
                try:
                    ...
                    scripts=soup.find_all(name='div',attrs={"class":re.compile(r'.*(foot|nav|Nav|footer|bottom|pageR_t clearfix|menu|header).*$')})
                    scriptsId=soup.find_all(name='div',attrs={"id":re.compile(r'.*(foot|nav|Nav|footer|bottom|pageR_t clearfix|header|guest).*$')})
