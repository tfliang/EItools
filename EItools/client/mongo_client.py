...
class MongoDBClient(object):
    ...
    def get_collection(self, collection_name, db_name=settings.MONGO_DBNAME, user=settings.MONGO_USERNAME,
    ...
    def get_all_task(self, offset=0, size=0):
        ...
        if size > 0 and offset >= 0:
            tasks = self.get_collection(settings.MONGO_ALL_TASK).find().sort("publish_time", pymongo.DESCENDING).skip(
                offset).limit(size)
        else:
    ...
    def search_crawled_person_by_taskId(self, id, name, offset=0, size=0):
        ...
        c = self.get_collection(settings.MONGO_CRAWLED_PERSON).find(
            {'$and': [{"task_id": ObjectId(id)}, {"name": re.compile(name)}]})
        if size > 0 and offset >= 0:
            c = c.skip(offset).limit(size)
        for item in c:
            item['id'] = str(item['_id'])
    ...
    def search_crawled_person_num_by_taskId(self, id, name):
        return self.get_collection(settings.MONGO_CRAWLED_PERSON).find(
            {'$and': [{"task_id": ObjectId(id)}, {"name": re.compile(name)}]}).count()
    ...
    def get_person_num_by_taskId(self, id):
        return self.get_collection(settings.MONGO_UNCRAWLED_PERSON).find(
            {"$and": [{"task_id": ObjectId(id)}, {"status": {'$ne': 0}}]}).count()
    ...
    def get_changeinfo_list(self, id=None, offset=0, size=0):
        if id is not None:
            c = self.get_collection(settings.MONGO_CRAWLED_PERSON).find(
                {'$and': [{"changed": True}, {"_id": ObjectId(id)}]},
                {'change_info': 1, 'task_id': 1, '_id': 1, 'name': 1})
        else:
            c = self.get_collection(settings.MONGO_CRAWLED_PERSON).find({"changed": True},
                                                                        {'change_info': 1, 'task_id': 1, '_id': 1,
                                                                         'name': 1})
        if size > 0 and offset >= 0:
            c = c.skip(offset).limit(size)
        persons = []
    ...
    def search_changeinfo_list(self, name=None, offset=0, size=0):
        if name is not None:
            c = self.get_collection(settings.MONGO_CRAWLED_PERSON).find(
                {'$and': [{"changed": True}, {"name": re.compile(name)}]},
                {'change_info': 1, 'task_id': 1, '_id': 1, 'name': 1})
        else:
            c = self.get_collection(settings.MONGO_CRAWLED_PERSON).find({"changed": True},
                                                                        {'change_info': 1, 'task_id': 1, '_id': 1,
    ...
    def search_changeinfo_list_num(self, name):
        return self.get_collection(settings.MONGO_CRAWLED_PERSON).find(
            {'$and': [{"changed": True}, {"name": re.compile(name)}]}).count()
    ...
    def update_person_by_id(self, person_id, task_id):
        self.get_collection(settings.MONGO_UNCRAWLED_PERSON).update({"_id": ObjectId(person_id)},
                                                                    {"$set": {"status": 0}})
